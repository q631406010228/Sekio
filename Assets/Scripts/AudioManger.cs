...
public class AudioManger {
    ...
    public void PlayAudioUI(string clipName, Transform parent)
    {
        AudioSource source;
        if (gameObjectUI == null)
        {
            gameObjectUI = new GameObject();
            source = gameObjectUI.AddComponent<AudioSource>();
            gameObjectUI.transform.SetParent(parent);
            gameObjectUI.transform.localPosition = Vector3.zero;
        }
        else
            source = gameObjectUI.GetComponent<AudioSource>();
        AudioClip tempPlayClip = CheckPlayClip(clipName);
